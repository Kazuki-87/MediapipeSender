...
def get_pose_image(frame):
    ...
    if results_face.multi_face_landmarks:
        for face_landmarks in results_face.multi_face_landmarks:
            expression_data.append([(f"FaceLandmark_{i}", lmk.x, lmk.y, lmk.z) for i, lmk in enumerate(face_landmarks.landmark)])
            # 顔のランドマークを描画
            mp_drawing.draw_landmarks(image, face_landmarks, mp_face_mesh.FACEMESH_CONTOURS)

    if results_hands.multi_hand_landmarks and results_hands.multi_handedness:
        fingertip_data = []  # 両手の指先データを格納するリスト
        for hand_landmarks, handedness in zip(results_hands.multi_hand_landmarks, results_hands.multi_handedness):
            hand_label = handedness.classification[0].label  # "Left" または "Right"
            # 指先のランドマークインデックス (例: Thumb_tip, Index_tip, Middle_tip, Ring_tip, Pinky_tip)
            fingertip_indices = [4, 8, 12, 16, 20]
            fingertip_data.append([(f"{hand_label}_Fingertip_{i}", hand_landmarks.landmark[i].x, 
                                    hand_landmarks.landmark[i].y, 
                                    hand_landmarks.landmark[i].z) for i in fingertip_indices])
            hand_data.append([(f"{hand_label}_HandLandmark_{i}", lmk.x, lmk.y, lmk.z) for i, lmk in enumerate(hand_landmarks.landmark)])
            # 指先ランドマークを描画
